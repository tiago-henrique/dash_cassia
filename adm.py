...
with col2:
    # Calculando dados
    bundle_completo_perc = db_clean_filtrado[db_clean_filtrado['Bundle Completo'].isin(valores)]['Bundle Completo'].value_counts().reset_index()
    ...
    if bundle_completo_perc.empty or bundle_completo_perc.shape[1] <= 1:
        ...
        #Determinar o valor da linha de meta - 80
        linha_meta_bundle = (80 / 100) * total_atendidos
        linha_meta_bundle = round(linha_meta_bundle,2)
        
        bundle_meta = 'A meta foi atingida' if valor_bundle > linha_meta_bundle else 'A meta não foi atingida'
        cor = "green" if valor_bundle > linha_meta_bundle else "red"
    ...
    fig_bundle.add_hline(
        ...
        line_dash='dash',
        line_color='red',
        annotation_text=f'Meta: 80% -  ({linha_meta_bundle})',
...
with col3:
    ...
    if atb_perc.empty or atb_perc.shape[1] <= 1:
        ...
        atb_meta = 'A meta foi atingida' if valor >= total_atendidos else 'A meta não foi atingida'
        cor = "green" if valor >= total_atendidos else "red"
